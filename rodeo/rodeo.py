...
class RodeoRenderer(object):
    def render(self, template, context=None, strict=False, **kw):
        """
        Render a rodeo template (data).
    
        :param template: The template data to render.
        ...
        :returns: string
        """
        txt = pystache.render(template, context, **kw)
        txt = self._bless(txt, strict)
        return txt

    def _termify(self, func, txt, raise_error=False):
        """
    ...
    def _bless(self, txt, strict=False):
        """
        ...
        for line in StringIO(txt).readlines():
            ...
            for part in parts:
                ...
                if full_block:
                    ...
                    block_txt = self._termify(func, full_block.group(4), strict)
                    full_txt = "%s%s%s" % (
                        full_block.group(1),
                        block_txt,
                        full_block.group(6), 
                ...
                if end_block:
                    ...
                    end_txt = "%s%s" % (
                        self._termify(func, block_txt, strict),
                        end_block.group(3),
                        )
...
def render(template, context=None, strict=False, **kw):
    """
    Render a rodeo template (data) (wrapper around RodeoRenderer).
    ...
    """
    rr = RodeoRenderer()
    txt = rr.render(template, context, **kw)
    return txt
...
def render_file(template_path, context=None, strict=False, **kw):
    ...
    data to rodeo.render().

    :param template_path: The template path to render.
    :param context: The context dictionary to pass to the template.
    :param strict: Whether or not to raise errors during parsing.
    :param kw: Additional keyword arguments to pass to PyStache.
    :returns: string
